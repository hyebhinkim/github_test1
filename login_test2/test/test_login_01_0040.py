...
def test_login_01_0040(driver_fixture):
    LabelEarth(driver_fixture)
    login = Login(driver_fixture)

    step_1_fail_message = "ID가 제대로 입력됨"
    login.id_input.send_keys(FAKE_ID)
    assert login.id_input.get_attribute("value") == FAKE_ID, step_1_fail_message
