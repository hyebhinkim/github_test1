...
from login.ui.elements import TEST_ID, TEST_PW, ADMIN_PW
...
def test_mypage_01_0210(driver_fixture):
    ...
    step_2_fail_message = "새 비밀번호 불일치 에러 메세지가 보여지지 않음"
    edit.edit_info.click()
    edit.edit_password.click()
    edit.current_password.send_keys(TEST_PW)
    edit.new_password.send_keys(TEST_PW)
    edit.new_password_confirm.send_keys(ADMIN_PW)
    edit.change_btn.click()
    assert is_text_present(driver_fixture, '새 비밀번호가 일치하지 않습니다.'), step_2_fail_message
